...
def run_exp():
    exec_path_lst = [
            ...
            '/homes/ywangby/workspace/yche/git-repos/SimRank/LPMC-fgi/build/carmo-h-121']
                ...
                for k in [100]:
                    for fgi in xrange(0,10000001,2000000):
                        for repeat_cnt in xrange(3):
                            ...
                            params_lst = map(str, [algo_path, dataset_name, q, 0, k, 0.01, fgi, 100, '>>', stat_file_path])
                            cmd = ' '.join(params_lst)

                            time_out = 1200
                            tle_flag, info, correct_info = time_out_util.run_with_timeout(cmd, timeout_sec=time_out)
                            # print info, correct_info
