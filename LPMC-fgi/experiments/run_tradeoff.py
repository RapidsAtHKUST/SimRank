...
def run_exp():
    exec_path_lst = [
            ...
            #'/homes/ywangby/workspace/yche/git-repos/SimRank/SPS-Variants/par-sr/build/par-sr']
    ...
    large_dataset_lst = [
            ...
            'wiki-Link']
    tag = 'exp_results'
    folder_name = 'tradeoff'

    for algo_path in exec_path_lst:
        for dataset_name in small_dataset_lst:
            for q in [10 ** 5]:#, 10 ** 6, 10 ** 7]:
                for k in [100]:
                    for eps in [0.002,0.004,0.006,0.008,0.01,0.012,0.014,0.016,0.018,0.02]:
                        for delta in [0.002,0.004,0.006,0.008,0.01,0.012,0.014,0.016,0.018,0.02]:
                            for repeat_cnt in xrange(3):
                                algo_name = algo_path.split('/')[-1]
                                stat_dir = os.sep.join(map(str, ['.', tag, folder_name, q, k, eps, delta, dataset_name]))
                                ...
                                params_lst = map(str, [algo_path, dataset_name, q, repeat_cnt, k, eps, delta, '>>', stat_file_path])
                                cmd = ' '.join(params_lst)
                                os.system(cmd)
