...
root_path = '/homes/ywangby/workspace/yche/git-repos/SimRank/LPMC-fgi/experiments/exp_results/tradeoff'

def get_file_path(q, k, eps, delta,  dataset_name, algo_name):
    return os.sep.join(map(str, [root_path, q, k, eps, delta, dataset_name, algo_name + '.txt']))
...
#    'par-sr']
    ...
    eps_lst = [0.002,0.004,0.006,0.008,0.01,0.012,0.014,0.016,0.018,0.02]
    delta_lst = [0.002,0.004,0.006,0.008,0.01,0.012,0.014,0.016,0.018,0.02]
                ...
                for delta in delta_lst:
                    ...
                    for tag in tag_lst:
                        if tag == 'ndcg' or tag == 'precison':
                            tag_data_lst.append(get_tag_info(get_file_path(10 ** 5, 100, eps, delta, dataset_name, algo_name), tag, max))
                        else:
                            tag_data_lst.append(get_tag_info(get_file_path(10 ** 5, 100, eps, delta, dataset_name, algo_name), tag, min))
                    delta_data_lst.append(dict(zip(tag_lst, tag_data_lst)))
                eps_data_lst.append(dict(zip(delta_lst, delta_data_lst)))
            dataset_data_lst.append(dict(zip(eps_lst, eps_data_lst)))
        algo_data_lst.append(dict(zip(small_dataset_lst, dataset_data_lst)))


    with open(os.sep.join([data_folder, 'tradeoff.json']), 'w') as ofs:
