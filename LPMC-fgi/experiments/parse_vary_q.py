...
root_path = '/homes/ywangby/workspace/yche/git-repos/SimRank/LPMC-fgi/experiments/exp_results/vary_q'
...
if __name__ == '__main__':
    ...
    q_lst = xrange(50000, 150001, 25000)
            ...
            for q in q_lst:
                ...
                for tag in tag_lst:
                    if tag == 'ndcg' or tag == 'precison':
                        tag_data_lst.append(get_tag_info(get_file_path(q, 100, 0.01, dataset_name, algo_name), tag, max))
                    else:
                        tag_data_lst.append(get_tag_info(get_file_path(q, 100, 0.01, dataset_name, algo_name), tag, min))
                q_data_lst.append(dict(zip(tag_lst, tag_data_lst)))
            dataset_data_lst.append(dict(zip(q_lst, q_data_lst)))
        algo_data_lst.append(dict(zip(large_dataset_lst, dataset_data_lst)))


    with open(os.sep.join([data_folder, 'vary_q_58.json']), 'w') as ofs:
