...
def run_exp():
    ...
    sample_num_lst = [10 ** 6, 10 ** 6, 10 ** 6, 10 ** 6, 10 ** 6, 10 ** 5]
    ...
    c = 0.6
    delta_lst = [10 ** (-(i + 1)) for i in xrange(9)]
    eps = 0.01
        ...
        for algorithm_path in exec_path_lst:
            for delta in delta_lst:
                ...
                statistics_dir = os.sep.join(
                    map(str, ['.', tag, folder_name, data_set_name, sample_num_dict[algorithm_path], delta]))
