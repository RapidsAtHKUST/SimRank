...
def draw_query_index_time():
    ...
    for idx, algorithm in enumerate(algorithm_tag_lst):
        time_lst = list(map(lambda delta: delta_query_dict[algorithm][str(delta)], delta_lst))
    ...
    lst_lst = []
    for idx, algorithm in enumerate(algorithm_tag_lst):
        time_lst = list(map(lambda delta: delta_index_dict[algorithm][str(delta)], delta_lst))
        if algorithm in [tsf_tag]:
    ...
    ax.set_xticks(list(map(lambda idx: delta_lst[idx], [0, 2, 4, 6, 8])))
    for tick in ax.yaxis.get_major_ticks():
        tick.label.set_fontsize(TICK_SIZE + large_size_plus)
    for tick in ax.xaxis.get_major_ticks():
        tick.label.set_fontsize(TICK_SIZE + large_size_plus)
...
if __name__ == '__main__':
    ...
    #     print
    #     algorithm, delta_query_dict[algorithm]

    # print
    index_lst = [flp_tag, sling_tag, tsf_tag]
    # for algorithm in index_lst:
    #     print
    #     algorithm, delta_index_dict[algorithm]
