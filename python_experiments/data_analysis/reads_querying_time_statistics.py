from reads_indexing_statistics import *
...
def get_time_lst(data_set, pair_num, round_num, result_file_name):
    ...
    for round_i in xrange(round_num):
        ...
        cpu_time_lst.append(get_tag_info(my_path, query_time_tag))
    return cpu_time_lst
...
def get_query_time(data_set, algorithm_name, pair_num, round_idx):
    ...
    indexing_time = get_tag_info(file_path, 'total query cpu time')
    return float(format_str(indexing_time)) / 2.
