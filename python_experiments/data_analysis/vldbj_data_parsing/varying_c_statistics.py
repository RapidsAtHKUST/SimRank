...
def get_file_path(c, algorithm, pair_num=10 ** 6):
    root_path = '/home/yche/mnt/wangyue-clu/csproject/biggraph/ywangby/yche/' \
                'git-repos/SimRank/python_experiments/exp_results/varying_c_exp' if algorithm is not probesim_tag \
        else '/home/yche/mnt/wangyue-clu/csproject/biggraph/ywangby/yche/new-git-repos-yche/SimRank/' \
             'python_experiments/exp_results/varying_c_exp'
...
def get_indexing_time_lst(algorithm, pair_num=10 ** 6):
    def get_index_time(c):
        if algorithm in [bflpmc_tag, flpmc_tag]:
            return get_tag_info(get_file_path(c, algorithm, pair_num), 'total indexing cost', min)
        elif algorithm in [sling_tag]:
            cal_d_parallel_time = get_tag_no_colon_info(get_file_path(c, algorithm, pair_num), 'finish calcD', min)
            cal_h_time = get_tag_no_colon_info(get_file_path(c, algorithm, pair_num), 'finish backward', min)
            return cal_d_parallel_time * 6 + cal_h_time
        elif algorithm in [tsf_tag]:
            return get_tag_info(get_file_path(c, algorithm, pair_num), 'indexing computation time', min)
...
if __name__ == '__main__':
    ...
    def stat_without_probesim():
        ...
        def get_time_lst():
            ...
            with open(os.sep.join([data_folder, 'varying_c_query.json']), 'w') as ofs:
        ...
        def get_index_lst():
            ...
            with open(os.sep.join([data_folder, 'varying_c_index.json']), 'w') as ofs:
    ...
    def stat_with_probesim():
        ...
        def get_time_lst():
            ...
            with open(os.sep.join([data_folder, 'probesim_varying_c_query.json']), 'w') as ofs:
                ofs.write(json.dumps(dict(zip(algorithm_lst, time_lst)), indent=4))
