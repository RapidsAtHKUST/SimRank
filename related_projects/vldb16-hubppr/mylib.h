...
//     cpu_set_t mask; // a cpu set, each bit denotes whether this cpu is included
// 	CPU_ZERO(&mask); //init cpu set, each bit is set to be 0
	
// 	if(i < sysconf(_SC_NPROCESSORS_CONF)){
// 		CPU_SET(i,&mask); // add cpu i to the cpu set
		
// 		// if(-1 == sched_setaffinity(syscall(__NR_gettid),sizeof(&mask),&mask)){ //bind a thread with the cpu set
//         pthread_t thread = pthread_self();
//         if(0!=pthread_setaffinity_np(thread, sizeof(cpu_set_t), &mask)){
// 			return -1;
// 		}
// 	}
// 	return 0;
