...
struct BackPush{ // Backward Push 
    ...
    pair<double, int> backward_push(NodePair np, unique_max_heap& ); // self-adaptive backward local push, return estimate
    ...
    int sample_one_pair(NodePair np, std::default_random_engine& generator, std::uniform_real_distribution<double>& dist); // sample one pair of random walk
    ...
    bool is_keep_on_push(unique_max_heap &heap); // the indicator bool function to decide whether the local push would continue
