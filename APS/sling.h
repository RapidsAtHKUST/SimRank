...
bool cmpTuple(const std::tuple<int, int, int, double> &t1, const std::tuple<int, int, int, double> &t2) {
...
private:
    ...
    vector<double>
    pushback_q(map<pair<int, int>, double, PairCmp>::iterator start, map<pair<int, int>, double, PairCmp>::iterator end,
               double eps, int T);
...
public:
    vector<std::tuple<int, int, int, double> > p;
...
void Sling::t_backward(double eps, mutex *tasklock, int *cursor, int tid, mutex *plock) {
    int s, t;
    vector<std::tuple<int, int, int, double> > vec;
    while (true) {
        vec.clear();
        tasklock->lock();
        s = *cursor;
...
//                    vec.push_back(make_std::tuple(u, t, v, value));
...
vector<double> Sling::pushback_q(map<pair<int, int>, double, Sling::PairCmp>::iterator start,
                                 map<pair<int, int>, double, Sling::PairCmp>::iterator end, double eps, int T) {
