...
#define gmap google::sparse_hash_map
#define gset google::sparse_hash_set
...
private:
    ...
    const static int NUMTHREAD = 1; // 
    class Rand
    {
    private:
        sfmt_t sfmt[NUMTHREAD];
    public:
        Rand() { for (int i = 0; i < NUMTHREAD; ++i) sfmt_init_gen_rand(sfmt + i, std::rand()); }
    ...
    {
    public:
        size_t operator()(const pair<int, int>& p) const { hash<int> inthash; return inthash(p.first) + inthash(p.second); }
    };
    ...
    vector<double> pushback_q(map<pair<int, int>, double, PairCmp>::iterator start, map<pair<int, int>, double, PairCmp>::iterator end, double eps, int T);
...
public:
    ...
    Sling(Graph* g, double c) : g(g), c(c), sqrtc(sqrt(c)), d(NULL) { init(); }
    void init();
    ~Sling(){ delete [] d; delete [] first; delete [] second; }
...
double Sling::calcDi_2(int i, double eps, bool& early, int& R, int tid)
{
    ...
    if (X / (double) R <= eps)
    {
        return 1. - c * c * c * (X / (double) R) * (double) (in_i.size() - 1) / (double) in_i.size() - c * c * p - c / (double) in_i.size();
    }
...
map<pair<int, int>, double, Sling::PairCmp> Sling::pushback(int u, double eps, int tid)
{
    map<pair<int, int>, double, Sling::PairCmp> p;
    gmap<int, double> pt;
    gmap<int, double> ptt;
    gset<int> s;
    deque<int> q;
...
vector<double> Sling::pushback_q(map<pair<int, int>, double, Sling::PairCmp>::iterator start, map<pair<int, int>, double, Sling::PairCmp>::iterator end, double eps, int T)
{
...
double Sling::simrank(int u, int v)
{
    ...
    gmap<pair<int, int>, double, PairHash> pu;
...
vector<double> Sling::simrank(int u)
{
    ...
    map<pair<int, int>, double, Sling::PairCmp> pu;
    for (long long i = pstart[u]; i < pstart[u + 1]; ++i)
        pu[make_pair(std::get<1>(p[i]), std::get<2>(p[i]))] = std::get<3>(p[i]);
