...
public:
    ...
    MC_Tree createMCTree(int nodeId) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < length2; j++) {
                ...
                mc_tree.map2[tempNode2].push_back(
                        pair<int, double>(tempNode, C_value * C_value / (double) length / (double) length2));
            }
        }
        return mc_tree;
    }
    ...
    void t_singlePair(int u, int v, int walk_num, int min_meet_dist, int tid) {
        ...
        clock_t t1 = clock();
        cout << "thread: " << tid << ", walk: " << walk_num << ", time: " << (t1 - t0) / (double) CLOCKS_PER_SEC
             << endl;
    }
    ...
    void poll(string query_file, int num_query, int k, vector<string> dirs, string poll_gt_dir) {
        ...
        for (int cnt = 0; cnt < num_query; cnt++) {
            ...
            sort(mc_gt.begin(), mc_gt.end(), [](const pair<int, double> &a, const pair<int, double> &b) {
                return a.second > b.second;
            });
            stringstream ss_gt;
