...
void SerialNaiveSim::InitGraph(string &file_path) {
    n = 0u;
    auto edge_lst = GetEdgeList(file_path);
#ifdef DEBUG
    cout << edge_lst << endl;
#endif
    auto in_deg_dict = spp::sparse_hash_map<uint32_t, uint32_t>();
    ...
    for (auto i = 0u; i < n; i++) {
        in_deg_arr[i] = in_deg_dict.contains(i) ? in_deg_dict[i] : 0u;
    }
...
SerialNaiveSim::SerialNaiveSim(string &file_path) {
    InitGraph(file_path);
...
vector<double> &SerialNaiveSim::ComputeSim(int max_iter, double eps, double c) {
    int prev_idx = 1;
    cur_idx = 0;
            ...
            for (auto b = 0; b < n; b++) {
                ...
                for (auto off_a = off[a]; off_a < off[a + 1]; off_a++) {
                    ...
                    for (auto off_b = off[b]; off_b < off[b + 1]; off_b++) {
                        ...
                        if (nei_a != nei_b) {
                            cur_sim_mat[nei_a * n + nei_b] +=
                                    c / (in_deg_arr[nei_a] * in_deg_arr[nei_b]) * prev_sim_mat[a * n + b];
                        }
                    }
                }
            }
